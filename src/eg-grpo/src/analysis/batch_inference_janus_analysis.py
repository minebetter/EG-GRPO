...
import sys
work_dir = '/mnt/petrelfs/songhan/cot/t2i-r1/src/t2i-r1/src'
...
def generate(
    ...
    for j in range(0, inputs_embeds.shape[0], batch_size):
        ...
        for jj in range(0, inputs_embeds_img.shape[0], split_size):
            ...
            print('generated_tokens',generated_tokens.shape,'len(total_generated_tokens_img)',len(total_generated_tokens_img))
            entropy_generations.append(torch.cat(entropy_img_batch, dim=1))
            print('entropy_generations',entropy_generations[-1].shape)
    ...
    print('total_generated_tokens_img',total_generated_tokens_img.shape)
    ...
    for i, (reward_func, reward_processing_class) in enumerate(
        ...
        for key in reward_kwargs:
            for input_example in example:
                # repeat the key for (num_generation * new_generations_image) times
    ...
    return rewards_per_func, entropy_imag_mean, entropy_imag_std, mark
...
def main(args):
    ...
    with open(rank_save_path, "w", encoding="utf-8") as fout:
        for example in tqdm(local_data, desc=f"Processing on {accelerator.process_index}"):
            ...
            final_prompt = sft_format
            rewards, entropy_imag_mean, entropy_imag_std, mark = generate(
                    vl_gpt,
                    vl_chat_processor,
                    final_prompt,
                    prompt_text,
            ...
            print('rewards',rewards.shape,'entropy_imag_mean',entropy_imag_mean.shape)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reasoning_prompt_path", type=str,help="Path to the cot reasoning prompt")
