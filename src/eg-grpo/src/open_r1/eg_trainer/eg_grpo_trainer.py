...
class JanusT2IR1Trainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        with torch.no_grad():
            # Move entropy tensors to CPU for sorting-based threshold computation
            entropy_txt_cpu = entropy_txt.detach().cpu()
            entropy_img_cpu = entropy_img.detach().cpu()

            # Sort entropy values along the token dimension (ascending order)
            entropy_txt_sorted, _ = entropy_txt_cpu.sort(dim=1)
            entropy_img_sorted, _ = entropy_img_cpu.sort(dim=1)

        # Count the number of valid (non-zero) text tokens per sample
        txt_nonzero_len= (entropy_txt_cpu > 0).sum(dim=1)

        # High-entropy threshold for text tokens:
        # select a position corresponding to 50% of valid tokens,
        # but indexed from the tail of the sorted full-length tensor
        idx_txt_re = (txt_nonzero_len.float() * 0.5).clamp(min=1).long() 
        gather_idx = (entropy_txt_sorted.size(1) - idx_txt_re).unsqueeze(1) 
        entropy_limit_txt = entropy_txt_sorted.gather(1, gather_idx)

        # Low-entropy threshold for text tokens:
        # select a lower percentile (20% of valid tokens), mapped into the full-length index space
        ...
        # High-entropy and low-entropy thresholds for image tokens:
        # computed directly from the full sorted image-token entropy distribution
        ...
        # Construct high-entropy masks and low-entropy masks for text and image tokens
        ...
        # Log mean entropy statistics for monitoring
