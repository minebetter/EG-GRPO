...
class JanusT2IR1Trainer(Trainer):
    ...
    def __init__(
        ...
        for name, param in model.named_parameters():
            if name.startswith("vision_model") or name.startswith("aligner") or name.startswith("gen"): # choose whatever you like here
                ...
                param_train.append(param)
        assert len(param_train) > 0, "param_train should not be smaller than zero" 
        ...
        if is_deepspeed_zero3_enabled() and args.beta != 0:
            if args.resume_training:
                assert args.resume_model_path is not None, 'resume model cannot be none'
                self.ref_model = AutoModelForCausalLM.from_pretrained(
                    args.resume_model_path, trust_remote_code=True
                )
                print(f"*****+++++*******&&&&&&&&$$$$$$\nresume model path is {args.resume_model_path}\n*******________+++++++++")
            else:
                self.ref_model = AutoModelForCausalLM.from_pretrained(
        ...
        #print('self.ref_model:',self.ref_model)
        # Processing class
        ...
        #('self.image_token_num_per_image',self.image_token_num_per_image)
    ...
    def _get_per_token_logps_and_entropy(self, model, input_embeds, text_ids, img_ids, attention_mask, chunk_T=256):
    # -------- Differentiable: compute per-token log-probabilities in chunks (avoid allocating a huge log_softmax tensor) --------
        def _logps_from_logits_chunked_diff(logits, input_ids, chunk_T: int = 256):
        # Align to (B, L-1)
            logits = logits[:, :-1, :]          # (B, L-1, V)
            input_ids = input_ids[:, 1:]        # (B, L-1)

            B, T, V = logits.shape
            out_logp = torch.empty((B, T), device=logits.device, dtype=logits.dtype)

        # Use a more stable reduction dtype while keeping gradients (casting does not break autograd)
            red_dtype = torch.float32

            for s in range(0, T, chunk_T):
                e = min(T, s + chunk_T)
                L_blk = logits[:, s:e, :]                     # Do not detach, keep it differentiable
                m = L_blk.max(dim=-1, keepdim=True).values    # [B,t,1]
                x = (L_blk - m).to(red_dtype)                 # [B,t,V], differentiable

                logsum = torch.logsumexp(x, dim=-1)           # [B,t]，differentiable
                logZ = m.squeeze(-1).to(red_dtype) + logsum   # [B,t]

                ids_blk = input_ids[:, s:e]                   # [B,t]
                # Gather in the original dtype for better accuracy, then cast to red_dtype to combine with logZ
                gather_logits = L_blk.gather(-1, ids_blk.unsqueeze(-1)).squeeze(-1).to(red_dtype)  # [B,t]

                out_logp[:, s:e] = (gather_logits - logZ).to(logits.dtype)

            return out_logp  # [B, L-1], differentiable


    # -------- Non-differentiable: compute per-token entropy in chunks (no grad graph; lower memory / overhead) --------
        def _entropy_from_logits_chunked_nograd(logits, chunk_T: int = 256, normalize: bool = True):
             # Align to (B, L-1, V)
            logits = logits[:, :-1, :]

            B, T, V = logits.shape
            out_H = torch.empty((B, T), device=logits.device, dtype=logits.dtype)
            lnV = math.log(V)

            with torch.no_grad():                     # Do not build an autograd graph for the entire entropy computation
                red_dtype = torch.float32
                for s in range(0, T, chunk_T):
                    e = min(T, s + chunk_T)
                    L_blk = logits[:, s:e, :].detach().to(red_dtype)      # Explicitly detach from the graph
                    m = L_blk.max(dim=-1, keepdim=True).values            # [B,t,1]
                    x = L_blk - m                                         # [B,t,V]

                    expx   = torch.exp(x)                                 # [B,t,V]
                    sumexp = expx.sum(dim=-1)                             # [B,t]
                    logsum = torch.log(sumexp + 1e-20)                    # [B,t]
                    sumexp_x = (expx * x).sum(dim=-1)                     # [B,t]

                    H_blk = (logsum - sumexp_x / (sumexp + 1e-20))        # [B,t]
                    if normalize:
                        H_blk = H_blk / lnV
                    out_H[:, s:e] = H_blk.to(out_H.dtype)

            return out_H  # [B, L-1], non-differentiable

     # Forward pass to obtain hidden states (keep the computation differentiable)
        ...
        if img_ids is not None:
            ...
            # Concatenate a dummy token to align input ids 
            img_input_ids = torch.cat([img_ids.new_zeros(img_ids.size(0), 1), img_ids], dim=1)

            # Differentiable per-token log-probabilities 
            per_token_logps_img = _logps_from_logits_chunked_diff(image_logits, img_input_ids, chunk_T=chunk_T)

            # Non-differentiable per-token entropy 
            per_token_entropy_img = _entropy_from_logits_chunked_nograd(image_logits, chunk_T=chunk_T)

             # Pad to align sequence length 
            pad_len = input_embeds.size(1) - per_token_logps_img.size(1) - 1
            zeros_logp = per_token_logps_img.new_zeros((per_token_logps_img.size(0), pad_len))
            zeros_H    = per_token_entropy_img.new_zeros((per_token_entropy_img.size(0), pad_len))

            return torch.cat([zeros_logp, per_token_logps_img], dim=1), \
                torch.cat([zeros_H,    per_token_entropy_img], dim=1)

        else:
            # text logits
            text_logits = model.language_model.lm_head(last_hidden_states)

            per_token_logps_text = _logps_from_logits_chunked_diff(text_logits, text_ids, chunk_T=chunk_T)     # differentiable
            per_token_entropy_text = _entropy_from_logits_chunked_nograd(text_logits, chunk_T=chunk_T)         # non-differentiable

            return per_token_logps_text, per_token_entropy_text
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        #prompts = [[
        #        {"role": "User", "content": x["raw_prompt"]},
        #        {"role": "Assistant", "content": ""}] for x in inputs]
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            if self.max_textcot_length is not None:
                # print('self.max_textcot_length',self.max_textcot_length)
                # print("Before trim:", prompt_completion_ids.shape)
                prompt_completion_ids = prompt_completion_ids[:, -self.max_textcot_length :]         
                # print("After trim:", prompt_completion_ids[:, -10:])

            completion_ids = prompt_completion_ids
            # print("eos_token_id:", self.processing_class.tokenizer.eos_token_id)
            # print("completion_ids[-1]:", completion_ids[0, -10:])
        ...
        per_token_entropy = per_token_entropy[:,prompt_length - 1 :]
        txt_token_size = per_token_entropy.size(1)

        with torch.inference_mode():
            if self.ref_model is not None:
                self.ref_model.language_model.gradient_checkpointing_enable()
                ref_per_token_logps,ref_per_token_entropy = self._get_per_token_logps_and_entropy(
                    self.ref_model, 
                    input_embeds, 
                    prompt_all_ids, 
                    None,
                ...
                ref_per_token_entropy = ref_per_token_entropy[:, prompt_length - 1 :]
            else:
                # dummy ref_per_token_logps
                ref_per_token_logps = torch.zeros_like(per_token_logps)
                ref_per_token_entropy = torch.zeros_like(per_token_entropy)
        ...
        for i in range(completion_ids.shape[0]):
            ...
            #     print(f"final_prompt:{raw_prompt}. {answer}" )
        ...
        img_token_size = per_token_entropy.size(1)
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                #print('personally designed reward')
        ...
        per_token_logps, per_token_entropy, ref_per_token_logps, ref_per_token_entropy, completion_mask = [], [], [], [], []
        for key in ['semantic-cot', 'token-cot']:
            ...
            per_token_entropy.append(loss_dict[key]['per_token_entropy'])
            ref_per_token_logps.append(loss_dict[key]['ref_per_token_logps'])
            ref_per_token_entropy.append(loss_dict[key]['ref_per_token_entropy'])
            completion_mask.append(loss_dict[key]['completion_mask'])
            #print('key',key,'per_token_logps',per_token_logps.shape)
        per_token_logps = torch.cat(per_token_logps, dim=1)
        per_token_entropy = torch.cat(per_token_entropy,dim=1)
        ref_per_token_logps = torch.cat(ref_per_token_logps, dim=1)
        ref_per_token_entropy = torch.cat(ref_per_token_entropy,dim=1)
        completion_mask = torch.cat(completion_mask, dim=1)

        # Split per-token entropy into text tokens and image tokens.
        # For each part, compute entropy-based thresholds (approximate percentile boundaries)
        # that will be used to construct high-entropy and low-entropy masks.
        entropy_txt, entropy_img = torch.split(per_token_entropy, [txt_token_size, img_token_size], dim=1)

        def cpu_quantiles_chunked(x, mask=None, qs=(0.5, 0.2), chunk=32):
            B, T = x.shape
            qnum = len(qs)

            out = torch.empty((qnum, B, 1), dtype=torch.float32, device='cpu')

            for i in range(0, B, chunk):
                j = min(B, i + chunk)

                x_cpu = x[i:j].detach().to(dtype=torch.float32, device='cpu', non_blocking=True).numpy()

                if mask is not None:
                    m_cpu = mask[i:j].detach().to(device='cpu', non_blocking=True).numpy().astype(bool)
                    x_cpu = np.where(m_cpu, x_cpu, np.nan)
                    with np.errstate(all='ignore'): 
                        qvals = np.nanquantile(x_cpu, qs, axis=1, keepdims=True)
                else:
                    qvals = np.quantile(x_cpu, qs, axis=1, keepdims=True)

                out[:, i:j, :] = torch.from_numpy(np.asarray(qvals, dtype=np.float32))

            return out  


        with torch.no_grad():
            txt_mask = (entropy_txt > 0)

            txt_q_cpu = cpu_quantiles_chunked(entropy_txt, mask=txt_mask, qs=(0.5, 0.2), chunk=32)
            img_q_cpu = cpu_quantiles_chunked(entropy_img, mask=None,      qs=(0.5, 0.2), chunk=32)
            txt_q_cpu = torch.nan_to_num(txt_q_cpu, nan=0.0) 

            device = per_token_entropy.device
            dtype = per_token_entropy.dtype
            entropy_limit_txt      = txt_q_cpu[0].to(device=device,dtype=dtype)
            entropy_limit_txt_min  = txt_q_cpu[1].to(device=device,dtype=dtype)   
            entropy_limit_img      = img_q_cpu[0].to(device=device,dtype=dtype)
            entropy_limit_img_min  = img_q_cpu[1].to(device=device,dtype=dtype)


        entropy_txt_mask = entropy_txt > entropy_limit_txt
        entropy_img_mask = entropy_img > entropy_limit_img
        entropy_txt_mask_min = entropy_txt < entropy_limit_txt_min
        entropy_img_mask_min = entropy_img < entropy_limit_img_min
        entropy_txt_mask = entropy_txt_mask.float() 
        entropy_img_mask = entropy_img_mask.float()
        entropy_txt_zero_mask = 1. - entropy_txt_mask_min.float()
        entropy_img_zero_mask = 1. - entropy_img_mask_min.float()

        entropy_mask = torch.cat([entropy_txt_mask,entropy_img_mask],dim=1).to(per_token_logps.device)
        entropy_zero_mask = torch.cat([entropy_txt_zero_mask,entropy_img_zero_mask],dim=1).to(per_token_logps.device)

        del entropy_txt, entropy_img
        torch.cuda.empty_cache()
        ...
        # Add an entropy-based token-level bonus to the advantages
        advantages = advantages.unsqueeze(1)
        alpha,n_beta = 1, 0.3 #hyperparameter

        n_beta_use = n_beta #* (1 - 0.4*self.state.global_step/self.state.max_steps) 
        max_limit = advantages.abs()*0.3
        per_token_entropy = per_token_entropy.detach()
        mult_ratio = torch.min(max_limit,n_beta_use*per_token_entropy)
        #mult_ratio = alpha + n_beta_use*entropy_mask*per_token_entropy.detach()
        mult_ratio = mult_ratio*entropy_mask
        advantages = (advantages + mult_ratio)*entropy_zero_mask
        ...
        self._metrics[f"loss"].append(self.accelerator.gather_for_metrics(loss).mean().item())
