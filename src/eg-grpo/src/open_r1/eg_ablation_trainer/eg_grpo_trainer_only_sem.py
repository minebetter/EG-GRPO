...
class JanusT2IR1Trainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # print()
        if self.is_main_process():
            print('per_token_entropy',per_token_entropy.shape,'per_token_logps',per_token_logps.shape,'ref_per_token_logps',ref_per_token_logps.shape)
        #rewards = rewards/2 - 1
        ...
        idx_txt_re = (txt_nonzero_len.float() * 0.5).clamp(min=1).long() ##this number is reverse
        ...
        id_text_re_min = (txt_nonzero_len.float() * 0.2).clamp(min=1).long() ###attention, error,this original is 0.9
        ...
        #batch_pos_mask = (advantages > 0).float() 
        ...
        sem_mask = torch.zeros_like(entropy_mask)
        sem_mask[:,:txt_token_size] = 1
        entropy_zero_mask = 1-(1-entropy_zero_mask)*sem_mask
        mult_ratio = mult_ratio*entropy_mask*sem_mask
