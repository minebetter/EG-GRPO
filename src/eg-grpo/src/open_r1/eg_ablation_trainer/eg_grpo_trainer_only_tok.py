...
from utils.reward_gdino_new import GDino
...
class JanusT2IR1Trainer(Trainer):
    ...
    def __init__(
        ...
        # dataset_path = "/mnt/petrelfs/songhan/cot/t2i-r1/data/geneval_and_t2i_data_final.json"
        # self.tracked_prompts,self.reward_log_path = self.set_tracked_prompt(dataset_path)

    def set_tracked_prompt(self,dataset_path):  
          # 你数据集的路径
        with open(dataset_path, "r", encoding="utf-8") as f:
            all_data = [json.loads(line) for line in f]

        # 抽取 100 条不同 prompt
        sampled_data = random.sample(all_data, 100)
        tracked_prompts = set(d["prompt"] for d in sampled_data)

        # 建立一个记录字典，key=prompt，value=列表(每次训练的平均reward)
        tracked_rewards = {p: [] for p in tracked_prompts}

        # 建立保存结果的路径
        os.makedirs(self.args.output_dir, exist_ok=True)
        reward_log_path = os.path.join(self.args.output_dir, "tracked_rewards.csv")
        # 初始化 CSV 表头
        with open(reward_log_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["step", "prompt", "mean_reward"])
        return tracked_prompts,reward_log_path
    ...
    def _get_per_token_logps_and_entropy(self, model, input_embeds, text_ids, img_ids, attention_mask):
        def _get_per_token_logps_part(logits, input_ids):
            logits = logits[:, :-1, :]  # (B, L-1, V), exclude the last logit: it corresponds to the next token pred
            input_ids = input_ids[:, 1:]  # (B, L-1), exclude the first input ID since we don't have logits for it
            # Compute the log probabilities for the input tokens. Use a loop to reduce memory peak.
            ...
            for logits_row, input_ids_row in zip(logits, input_ids):
                ...
                per_token_logps.append(token_log_prob)
            return torch.stack(per_token_logps)
        def _get_per_token_entropy_part(logits):
            logits = logits[:,:-1,:]
        ...
        if img_ids is not None:
            ...
            img_input_ids = torch.cat([img_ids.new_zeros(img_ids.size(0), 1), img_ids], dim=1) # cat a random one here, since it is not used in the loss calculation
            per_token_logps_img = _get_per_token_logps_part(image_logits, img_input_ids) # only calculate image loss
            per_token_entropy_img = _get_per_token_entropy_part(image_logits)#image_logits.new_zeros(image_logits.size(0),image_logits.size(1)-1)#
            return torch.cat([
                per_token_logps_img.new_zeros(
                    (per_token_logps_img.size(0), input_embeds.size(1) - per_token_logps_img.size(1) - 1)
                ), # the return length should be the input length minus 1 (the last token does not need predict)
            ...
            dim=1),torch.cat([
                per_token_entropy_img.new_zeros( 
                    (per_token_logps_img.size(0), input_embeds.size(1) - per_token_logps_img.size(1) - 1)),
                per_token_entropy_img],
            dim=1)
        else: # only calculate text ids
            hidden_states = model.language_model(inputs_embeds=input_embeds, attention_mask=attention_mask, output_hidden_states=True).hidden_states  # (B, L, V)
            last_hidden_states = hidden_states[-1]
            ...
            per_token_logps_text = _get_per_token_logps_part(text_logits, text_ids) 
            per_token_entropy_text = _get_per_token_entropy_part(text_logits) #text_logits.new_zeros(text_logits.size(0),text_logits.size(1)-1)#
            return per_token_logps_text,per_token_entropy_text
    
    def _get_per_token_logps_and_entropy_pad(self, model, input_embeds, text_ids, img_ids, attention_mask):
        def _get_per_token_logps_part(logits, input_ids):
            logits = logits[:, :-1, :]  # (B, L-1, V), exclude the last logit: it corresponds to the next token pred
            input_ids = input_ids[:, 1:]  # (B, L-1), exclude the first input ID since we don't have logits for it
            # Compute the log probabilities for the input tokens. Use a loop to reduce memory peak.
        ...
        if img_ids is not None:
            ...
            per_token_entropy_text = _get_per_token_entropy_part(text_logits) #text_logits.new_zeros(text_logits.size(0),text_logits.size(1)-1)#
            return torch.cat([
                per_token_logps_text,per_token_logps_text.new_zeros(per_token_logps_text.size(0),max_txt_length-per_token_logps_text.size(1))]
                ,dim=1),torch.cat([
                    per_token_entropy_text,per_token_entropy_text.new_zeros(per_token_entropy_text.size(0),max_txt_length-per_token_entropy_text.size(1))],
                    dim=1)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # print("EOS位置:", eos_idx)
        # print("completion_mask.sum(dim=1):", completion_mask.sum(dim=1))
        ...
        #max_txt_size = self.max_completion_length
        #per_token_logps = torch.cat([per_token_logps,per_token_logps.new_zeros(per_token_logps.size(0),max_txt_size-per_token_logps.size(1))],dim=1)
        #per_token_entropy = torch.cat([per_token_entropy,per_token_entropy.new_zeros(per_token_logps.size(0),max_txt_size-per_token_entropy.size(1))],dim=1)
        #completion_mask = torch.cat([completion_mask,completion_mask.new_zeros(completion_mask.size(0),per_token_logps.size(1)-completion_mask.size(1))],dim=1)
        #print('self.ref_model is not None:',self.ref_model is not None)
        #assert completion_mask.size(1) == per_token_logps.size(1),'must add padding text embedding mask'
        with torch.inference_mode():
            if self.ref_model is not None:
                ...
                #ref_per_token_logps = torch.cat([ref_per_token_logps,ref_per_token_logps.new_zeros(ref_per_token_logps.size(0),max_txt_size-ref_per_token_logps.size(1))],dim=1)
                #ref_per_token_entropy = torch.cat([ref_per_token_entropy,ref_per_token_entropy.new_zeros(ref_per_token_entropy.size(0),max_txt_size-ref_per_token_entropy.size(1))],dim=1)
            else:
        ...
        for i in range(completion_ids.shape[0]):
            ...
            #print('++++++++++++===raw_prompt:',raw_prompt,'--------answer:',answer)
            #print('image_gen_prompt',image_gen_prompt)
        ...
        with torch.no_grad():
            entropy_txt_cpu = entropy_txt.detach().cpu()
            entropy_img_cpu = entropy_img.detach().cpu()

            entropy_txt_sorted, _ = entropy_txt_cpu.sort(dim=1)
            entropy_img_sorted, _ = entropy_img_cpu.sort(dim=1)

        txt_nonzero_len= (entropy_txt_cpu > 0).sum(dim=1)

        idx_txt_re = (txt_nonzero_len.float() * 0.5).clamp(min=1).long() 
        gather_idx = (entropy_txt_sorted.size(1) - idx_txt_re).unsqueeze(1)  # shape [B,1]
        entropy_limit_txt = entropy_txt_sorted.gather(1, gather_idx)
        id_text_re_min = (txt_nonzero_len.float() * 0.2).clamp(min=1).long() 
        gather_idx_min = (entropy_txt_sorted.size(1) - txt_nonzero_len + id_text_re_min).unsqueeze(1) 
        entropy_limit_txt_min = entropy_txt_sorted.gather(1, gather_idx_min)

        idx_img = int(entropy_img_sorted.size(1) * 0.5)
        entropy_limit_img = entropy_img_sorted[:, idx_img].unsqueeze(1)
        idx_img_min = int(entropy_img_sorted.size(1) * 0.2)
        entropy_limit_img_min = entropy_img_sorted[:, idx_img_min].unsqueeze(1)


        entropy_txt_mask = entropy_txt_cpu > entropy_limit_txt
        entropy_img_mask = entropy_img_cpu > entropy_limit_img
        entropy_txt_mask_min = entropy_txt_cpu < entropy_limit_txt_min
        entropy_img_mask_min = entropy_img_cpu < entropy_limit_img_min
        ...
        entropy_txt_mean = (entropy_txt * completion_mask[:,:txt_token_size]).sum() / completion_mask[:,:txt_token_size].sum()
        entropy_img_mean = (entropy_img * completion_mask[:,-img_token_size:]).sum() / completion_mask[:,-img_token_size:].sum()
        self._metrics["entropy_txt_mean"].append(entropy_txt_mean.item())
        self._metrics["entropy_img_mean"].append(entropy_img_mean.item())


        del entropy_txt_sorted, entropy_img_sorted, entropy_txt, entropy_img, entropy_txt_mask_min, entropy_img_mask_min
        torch.cuda.empty_cache()
        ...
        tok_mask = torch.zeros_like(entropy_mask)
        tok_mask[:,-img_token_size:] = 1
        mult_ratio = mult_ratio*entropy_mask*tok_mask
        entropy_zero_mask = 1-(1-entropy_zero_mask)*tok_mask
        ...
        #per_token_loss = per_token_loss  + self.beta/10 * per_token_entropy_kl
