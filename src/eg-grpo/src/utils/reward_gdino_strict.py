...
class GDino:
    ...
    def get_spatial_score(self, boxes_filt, pred_phrases, all_logits, nouns, spatial_info):
        score = 0
        obj1, obj2 = spatial_info['obj1'], spatial_info['obj2']
        nouns_check = [obj1, obj2]

        #print('boxes_filt, pred_phrases,all_logits', boxes_filt, pred_phrases, all_logits)
        for obj in nouns_check:
            count = pred_phrases.count(obj)
            #print(f'for obj {obj} count is {count}')
            if count > 0:
                score += 0.2
                if count > 1:
                    score -= min(0.1 * (count - 1), 0.2)
   
        if score >= 0.4:
            boxes_filt[:, :2] -= boxes_filt[:, 2:] / 2
            boxes_filt[:, 2:] += boxes_filt[:, :2]

            obj1_nms_boxes, obj2_nms_boxes = self.get_nms_boxes(
                boxes_filt, pred_phrases, all_logits, nouns_check, spatial_info
            )
            if obj1_nms_boxes.shape[0] == 1 and obj2_nms_boxes.shape[0] == 1:
                obj1_nms_boxes, obj2_nms_boxes = obj1_nms_boxes[0], obj2_nms_boxes[0]
                box1 = {"x_min": obj1_nms_boxes[0], "y_min": obj1_nms_boxes[1],
                        "x_max": obj1_nms_boxes[2], "y_max": obj1_nms_boxes[3]}
                box2 = {"x_min": obj2_nms_boxes[0], "y_min": obj2_nms_boxes[1],
                        "x_max": obj2_nms_boxes[2], "y_max": obj2_nms_boxes[3]}
                score += self.determine_position(spatial_info['locality'], box1, box2) / 2
    ...
    def get_object_score(self, boxes_filt, pred_phrases, all_logits, nouns):
        ...
        score = 0
        for noun in nouns:
            count = pred_phrases.count(noun)
            if count > 0:
                score += weight
                if count > 1:        
                    score_minus = min(weight * 0.5 * (count - 1), weight)      
                    score -= score_minus
                    #print(f'for noun{noun} count is {count}, and now score is {score_minus} and weight is {weight}')
        return max(0, score)
